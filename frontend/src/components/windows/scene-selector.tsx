...
const SceneSelector = observer((): JSX.Element => {
  ...
  const handleSceneSelect = (scene: IScene) => {
    ...
    sceneStore.fetchScene(scene.id);
  };
  ...
  const handleNewScene = async () => {
    ...
    try {
      const newScene = await sceneManagerApi.createScene({
        name: newSceneName.trim(),
      });
      await sceneStore.fetchScene(newScene.id);
      sceneStore.setSelectedScene(newScene);
      setNewSceneName("");
          ...
          <HoverCard key={scene.id} openDelay={200} closeDelay={200}>
            <HoverCardTrigger asChild>
              <button
                ...
                className="w-full aspect-square rounded-lg border border-gray-300 hover:bg-white hover:bg-opacity-25 p-4 transition-colors duration-200 flex flex-col items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-gray-200"
              >
