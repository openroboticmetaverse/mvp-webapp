...
import { Mesh, Object3D, PlaneGeometry, Scene, Vector3, MeshBasicMaterial, BufferAttribute, BufferGeometry, Group } from "three";
...
export class RobotManager {
  static RobotMap: Map<string, string> = new Map<string, string>([
    ["franka_arm", "https://raw.githubusercontent.com/openroboticmetaverse/mvp-test/master/assets/models/franka_description/robots/panda_arm_hand.urdf.xacro"],
    ["franka_dual_arm", "https://raw.githubusercontent.com/openroboticmetaverse/mvp-webapp/main/frontend/public/franka_description/robots/dual_panda_example.urdf.xacro"]
  ])
  ...
  addRobots(name: string, props: RobotProperty): Promise<void> {
    return new Promise(async (resolve, reject) => {
      ...
      props.position.forEach((pos) => {
        ...
        cloned.rotation.set(props.rotation.x, props.rotation.y, props.rotation.z);
        group.add(cloned);
  ...
  remove(id: number) {
    ...
    if (robot) {
      ...
      console.log(`Error:RobotManager:remove ==> robot with ID=${id} does not exist!`);
    }
  }
